...
//
// resumesv1alpha1Profile tests
//
func resumesv1alpha1ProfileChildrenFuncs(tester *E2ETest) error {
	// TODO: need to run r.GetResources(request) on the reconciler to get the mutated resources
	if len(resume.CreateFuncs) == 0 {
		return nil
	}

	workload, err := resume.ConvertWorkload(tester.workload)
	if err != nil {
		return fmt.Errorf("error in workload conversion; %w", err)
	}

	resourceObjects, err := resume.Generate(*workload)
	if err != nil {
...
func resumesv1alpha1ProfileNewHarness(namespace string) *E2ETest {
	return &E2ETest{
		namespace:          namespace,
		unstructured:       &unstructured.Unstructured{},
		workload:           &resumesv1alpha1.Profile{},
		sampleManifestFile: "../../config/samples/resumes_v1alpha1_profile.yaml",
		getChildrenFunc:    resumesv1alpha1ProfileChildrenFuncs,
		logSyntax:          "controllers.resumes.Profile",
	}
}

func (tester *E2ETest) resumesv1alpha1ProfileTest(testSuite *E2ECollectionTestSuite) {
...
func (testSuite *E2ECollectionTestSuite) Test_resumesv1alpha1Profile() {
	tester := resumesv1alpha1ProfileNewHarness("")
	tester.resumesv1alpha1ProfileTest(testSuite)
}
